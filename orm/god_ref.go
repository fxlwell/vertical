...
func NewRefModel(m interface{}) *Ref_Model {
	...
	if is_valid_model(m) {
		...
		vertical_util.Panicf("model must be ptr of struct. and must implement orm.Model")
	}
...
func (g *God) rows_to_model(rows *sql.Rows) (Model, error) {
	...
	for col, val := range datas {
		if field, ok := g.Model.Mapping_with_column[col]; ok {
			vertical_util.Assign_copy(r_model.FieldByName(field.R_StructField.Name), reflect.ValueOf(val))
		} else {
			if vertical_util.Is_byte_slice(val) {
...
func (g *God) rows_to_models(rows *sql.Rows) ([]Model, error) {
	...
	for {
		...
		for col, val := range datas {
			if field, ok := g.Model.Mapping_with_column[col]; ok {
				if vertical_util.Is_byte_slice(val) {
					vertical_util.Assign_copy(r_model.FieldByName(field.R_StructField.Name), reflect.ValueOf(string(val.([]byte))))
				} else {
					vertical_util.Assign_copy(r_model.FieldByName(field.R_StructField.Name), reflect.ValueOf(val))
				}
			} else {
